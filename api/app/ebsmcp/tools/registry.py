...
from app.ebsmcp.connectors.base import EBSConnector
from app.ebsmcp.identity import IdentityResolver, ResolvedIdentity, TargetSystem
...
def current_subject(ctx: ToolContext) -> str:
    ...
    """
    # Embedded-library path first: OraEBSAgent injects the authenticated
    # caller's subject (see app.ebsmcp.context). Falls back to the MCP
    # request token (standalone server) and then dev_subject, so the
    # vendored copy still behaves like the original when run standalone.
    from app.ebsmcp.context import get_injected_subject

    injected = get_injected_subject()
    if injected:
        return injected
